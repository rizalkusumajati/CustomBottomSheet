...
import com.example.custombottomsheet.SwipeableDefaults.resistanceConfig
...
@ExperimentalMaterialApi
internal fun <T> SwipeableState<T>.PreUpPostDownNestedScrollConnection(allowDragAndSwipe: Boolean): NestedScrollConnection
    = object : NestedScrollConnection {
        override fun onPreScroll(available: Offset, source: NestedScrollSource): Offset {
            val delta = available.toFloat()
            Log.d("BOTTOM", "ONPRESCROLL")
            return if (allowDragAndSwipe) {
        ...
        override suspend fun onPreFling(available: Velocity): Velocity {
            ...
            Log.d("BOTTOM", "ONPREFLING")
            return if(allowDragAndSwipe){
                if (toFling < 0 && offset.value > minBound) {
        ...
        override suspend fun onPostFling(consumed: Velocity, available: Velocity): Velocity {
            Log.d("BOTTOM", "ONPOSTFLING")
           if (allowDragAndSwipe) performFling(velocity = Offset(available.x, available.y).toFloat())
            return available
        }
